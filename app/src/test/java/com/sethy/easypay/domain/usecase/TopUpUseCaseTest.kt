...
class TopUpUseCaseTest {
    ...
    fun invoke_rejects_non_positive_amount() = runTest {
        val repo: WalletRepository = mock()
        try {
            useCase(repo).invoke(amountMajor = 0.0)
            org.junit.Assert.fail("Expected IllegalArgumentException for amount 0")
