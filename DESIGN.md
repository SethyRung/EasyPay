...
### Font Family
The system uses **Geist** (v1.7.2) as the single type family for the entire app — display and body both. **Geist Mono** (companion family, also in v1.7.2) handles code blocks. This is a deliberate simplification: rather than a serif/sans split, Geist's wide weight range (Regular 400, Medium 500, SemiBold 600) plus its negative-tracking display treatment at weight 400 carries the editorial voice. The cream canvas + generous tracking treatment + the wide weight range does the heavy lifting that a slab-serif would normally do.
